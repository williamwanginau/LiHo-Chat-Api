...
describe('Rooms e2e', () => {
  ...
  const mockPrisma = {
    $transaction: async (cb: (tx: any) => Promise<any>) => cb(mockPrisma as any),
    membership: {
    ...
    room: {
      ...
      create: jest.fn(async ({ data, select }: { data: { name: string; isPrivate?: boolean; memberships?: { create: { userId: string; role?: string } } }; select?: any }) => {
        const now = new Date();
